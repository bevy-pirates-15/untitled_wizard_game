...
pub(super) fn plugin(app: &mut App) {
    ...
    app.add_systems(
        Update,
        (spawn_chunks_around_camera, despawn_outofrange_chunks).chain().run_if(in_state(Screen::Playing)),
    );
}
...
fn spawn_chunk(commands: &mut Commands, images: &Res<ImageAssets>, chunk_pos: IVec2) {
    ...
    commands.entity(tilemap_entity).insert(TilemapBundle {
        ...
        texture: TilemapTexture::Single(images[&ImageAsset::MapTileset].clone()),
        tile_size: TILE_SIZE,
        transform,
